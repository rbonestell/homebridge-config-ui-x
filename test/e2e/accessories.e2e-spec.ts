...
import { SecretStoreService } from "src/core/secrets/secret-store.service";
import { KeyChainService } from "src/core/secrets/keychain.service";
...
describe("AccessoriesController (e2e)", () => {
  ...
  const mockedServices = [
    {
      ...
      uniqueId:
        "c8964091efa500870e34996208e670cf7dc362d244e0410220752459a5e78d1c",
    },
  ];
  ...
  beforeAll(async () => {
    ...
    let keyChainService = new KeyChainService(logger);
    let secretStoreService = new SecretStoreService(keyChainService, logger);
    let configService = new ConfigService(secretStoreService);
  ...
  beforeEach(async () => {
    ...
    // Setup mocks
    hapClientMock = vi
      .spyOn(accessoriesService.hapClient, "getAllServices")
    ...
    // authorization = await authService.signIn("admin", "admin");
  });
  ...
  it("PUT /accessories/:uniqueId (missing characteristic type)", async () => {
    ...
    expect(res.body).toContain(
      "characteristicType should not be null or undefined"
    );
  });
