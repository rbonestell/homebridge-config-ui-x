...
export class AccessoriesService {
  ...
  public async connect(client: any) {
    if (!this.configService.homebridgeInsecureMode()) {
      this.logger.error(
        "Homebridge must be running in insecure mode to control accessories."
      );
      return;
    }
  ...
  public async loadAccessories(): Promise<ServiceType[]> {
    if (!this.configService.homebridgeInsecureMode()) {
      throw new BadRequestException(
        "Homebridge must be running in insecure mode to access accessories."
      );
    }
    ...
    try {
      ...
      if (e.response?.status === 401) {
        this.logger.warn(
          "Homebridge must be running in insecure mode to view and control accessories from this plugin."
        );
      } else {
        this.logger.error(
          `Failed to load accessories from Homebridge as ${e.message}.`
        );
      }
      return [];
    }
  ...
  public async getAccessory(uniqueId: string) {
    ...
    if (!service) {
      throw new BadRequestException(
        `Service with uniqueId of '${uniqueId}' not found.`
      );
    }
   ...
   * @param value
   */
  public async setAccessoryCharacteristic(
    uniqueId: string,
    ...
    if (!characteristic || !characteristic.canWrite) {
      const types = service.serviceCharacteristics
        .filter((x) => x.canWrite)
        .map((x) => `'${x.type}'`)
        .join(", ");
      throw new BadRequestException(
        `Invalid characteristicType. Valid types are: ${types}.`
      );
    }

    // Integers
    if (
      ["uint8", "uint16", "uint32", "uint64"].includes(characteristic.format)
    ) {
      value = Number.parseInt(value as string, 10);
      if (
        characteristic.minValue !== undefined &&
        value < characteristic.minValue
      ) {
        throw new BadRequestException(
          `Invalid value. The value must be between ${characteristic.minValue} and ${characteristic.maxValue}.`
        );
      }
      if (
        characteristic.maxValue !== undefined &&
        value > characteristic.maxValue
      ) {
        throw new BadRequestException(
          `Invalid value. The value must be between ${characteristic.minValue} and ${characteristic.maxValue}.`
        );
    ...
    if (characteristic.format === "bool") {
      ...
      if (typeof value !== "boolean") {
        throw new BadRequestException(
          "Invalid value. The value must be a boolean (true or false)."
        );
      }
    }
  ...
  public async getAccessoryLayout(username: string) {
    try {
      const accessoryLayout = await readJson(
   ...
   * @param layout
   */
  public async saveAccessoryLayout(
    user: string,
    ...
    if (
      !(await pathExists(join(this.configService.storagePath, "accessories")))
    ) {
