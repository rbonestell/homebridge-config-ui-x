...
export class ConfigService {
  ...
  public runningInSynologyPackage = Boolean(
    process.env.HOMEBRIDGE_SYNOLOGY_PACKAGE === "1"
  );
  public runningInPackageMode = Boolean(
    process.env.HOMEBRIDGE_APT_PACKAGE === "1"
  );
  public runningInLinux =
    !this.runningInDocker &&
    !this.runningInSynologyPackage &&
    !this.runningInPackageMode &&
    platform() === "linux";
  public runningInFreeBSD = platform() === "freebsd";
  public canShutdownRestartHost =
    this.runningInLinux || process.env.UIX_CAN_SHUTDOWN_RESTART_HOST === "1";
  public enableTerminalAccess =
    (this.runningInDocker &&
  ...
  public recommendChildBridges = totalmem() > 2e9;
  ...
  public parseConfig(homebridgeConfig: HomebridgeConfig) {
    ...
    this.ui = Array.isArray(this.homebridgeConfig.platforms)
      ? this.homebridgeConfig.platforms.find((x) => x.platform === "config")
      : (undefined as any);
    ...
    this.secretToken = this.getSecretToken();
  ...
  public async uiRestartRequired(): Promise<boolean> {
    ...
    const currentPackage = await readJson(
      resolve(process.env.UIX_BASE_PATH, "package.json")
    );
    ...
    return !(
      isEqual(this.ui, this.uiFreeze) &&
      isEqual(this.homebridgeConfig.bridge, this.bridgeFreeze)
    );
  }
  ...
  private setConfigForDocker() {
    ...
    this.ui.theme =
      this.ui.theme || process.env.HOMEBRIDGE_CONFIG_UI_THEME || "orange";
    this.ui.auth =
      this.ui.auth ||
      (process.env.HOMEBRIDGE_CONFIG_UI_AUTH as "form" | "none") ||
      "form";
    this.ui.loginWallpaper =
      this.ui.loginWallpaper ||
      process.env.HOMEBRIDGE_CONFIG_UI_LOGIN_WALLPAPER ||
      undefined;
  }
  ...
  private getSecretToken(): string {
    this.migrateSecrets();
    return (
      this.secretStoreService.getSecret("JwtSecretKey") ??
      this.generateSecretToken()
    );
  }

  /**
   * Persists the secret token for signing JWTs
   * @param secret - The secret token for JWT signing
   */
  private setSecretToken(secret: string): void {
    this.secretStoreService.setSecret("JwtSecretKey", secret);
  }

  /**
   * Check for deprecated secrets file
  ...
  private generateSecretToken(): string {
    const newSecretToken = randomBytes(32).toString("hex");
    this.setSecretToken(newSecretToken);
  ...
  private getInstanceId(): string {
    return createHash("sha256").update(this.secretToken).digest("hex");
  }
  ...
  private async getCustomWallpaperHash(): Promise<void> {
    try {
      const fileStat = await stat(
        this.ui.loginWallpaper || this.customWallpaperPath
      );
      const hash = createHash("sha256");
      hash.update(
        `${fileStat.birthtime}${fileStat.ctime}${fileStat.size}${fileStat.blocks}`
      );
  ...
  private async checkIfRunningOnRaspberryPi() {
    try {
      this.runningOnRaspberryPi =
        (await pathExists("/usr/bin/vcgencmd")) &&
        (await pathExists("/usr/bin/raspi-config"));
    } catch (e) {
