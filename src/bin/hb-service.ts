...
import { getStoragePath } from "src/core/config/config.vars";
...
export class HomebridgeServiceHelper {
  ...
  constructor() {
    ...
    switch (platform()) {
      ...
      default:
        this.logger(
          `ERROR: This command is not supported on ${platform()}.`,
          "fail"
        );
        process.exit(1);
    }
    ...
    program
      ...
      .arguments(
        "[install|uninstall|start|stop|restart|rebuild|run|logs|view|add|remove]"
      )
      ...
      .option(
        ...
        "The name of the homebridge service to install or control",
      ...
      .option(
        ...
        (p) => (this.uiPort = Number.parseInt(p, 10))
      )
      .option(
        "--user <user>",
        "The user account the Homebridge service will be installed as (Linux, FreeBSD, macOS only)",
        (p) => (this.asUser = p)
      )
      .option(
        "--group <group>",
        "The group the Homebridge service will be added to (Linux, FreeBSD, macOS only)",
        (p) => (this.addGroup = p)
      )
  ...
  public logger(
    msg: string,
    level: "info" | "succeed" | "fail" | "warn" = "info"
  ) {
    if (this.action === "run") {
      msg =
        `\x1B[37m[${new Date().toLocaleString()}]\x1B[0m ` +
  ...
  private setEnv() {
    ...
    if (
      process.env.CONFIG_UI_VERSION &&
      process.env.HOMEBRIDGE_VERSION &&
      process.env.QEMU_ARCH
    ) {
      if (
        platform() === "linux" &&
        ["install", "uninstall", "start", "stop", "restart", "logs"].includes(
          this.action
        )
      ) {
        this.logger(
          `Sorry, the ${this.action} command is not supported in Docker.`,
          "fail"
        );
        process.exit(1);
      }
   ...
   */
  private showVersion() {
    const pjson = readJsonSync(resolve(getStoragePath(), "package.json"));
  ...
  private async launch() {
    if (
      ...
      this.logger(
        "Use the --allow-root flag to force the service to run as the root user."
      );
      process.exit(0);
    }
    ...
    if (cpus().length === 1 && arch() === "arm") {
      this.logger(
        "Delaying Homebridge startup by 20 seconds on low powered server."
      );
      setTimeout(() => {
        this.runHomebridge();
      }, 20000);
  ...
  private runHomebridge() {
    if (!this.homebridgeBinary || !pathExistsSync(this.homebridgeBinary)) {
      this.logger(
        "Could not find Homebridge. Make sure you have installed Homebridge using the -g flag then restart.",
        "fail"
      );
    ...
    if (this.homebridgeOpts.length) {
      this.logger(
        `Starting Homebridge with extra flags: ${this.homebridgeOpts.join(" ")}.`
      );
    }

    if (Object.keys(this.homebridgeCustomEnv).length) {
      this.logger(
        `Starting Homebridge with custom env: ${JSON.stringify(this.homebridgeCustomEnv)}.`
      );
    }
    ...
    this.homebridge = fork(
      this.homebridgeBinary,
      ["-C", "-Q", "-U", this.storagePath, ...this.homebridgeOpts],
      childProcessOpts
    );
    ...
    this.logger(
      `Started Homebridge v${this.homebridgePackage.version} with PID: ${this.homebridge.pid}.`
    );
  ...
  private async findHomebridgePath() {
    ...
    if (
      ...
      if (
        globalModules &&
        (await pathExists(resolve(globalModules, "homebridge")))
      ) {
    ...
    if (!this.homebridgeModulePath && process.env.UIX_CUSTOM_PLUGIN_PATH) {
      if (
        ...
        this.homebridgeModulePath = resolve(
          process.env.UIX_CUSTOM_PLUGIN_PATH,
          "homebridge"
        );
      }
    ...
    if (this.homebridgeModulePath) {
      try {
        ...
        return resolve(
          this.homebridgeModulePath,
          this.homebridgePackage.bin.homebridge
        );
      } catch (e) {
        console.log(e);
      }
  ...
  private async refreshHomebridgePackage() {
    try {
      if (await pathExists(this.homebridgeModulePath)) {
        this.homebridgePackage = await readJson(
          join(this.homebridgeModulePath, "package.json")
        );
      } else {
        this.logger(
          `Homebridge not longer found at ${this.homebridgeModulePath}.`,
          "fail"
        );
  ...
  private nodeVersionCheck() {
    ...
    if (Number.parseInt(process.versions.modules, 10) < 64) {
      this.logger(
        `Node.js v10.13.0 or greater is required, current: ${process.version}.`,
        "fail"
      );
      process.exit(1);
    }
  ...
  public async printPostInstallInstructions() {
    ...
    const defaultInterface = (await networkInterfaces()).find(
      (x: any) => x.iface === defaultAdapter
    );
  ...
  public async portCheck() {
    ...
    if (inUse) {
      this.logger(
        `Port ${this.uiPort} is already in use by another process on this host.`,
        "fail"
      );
      this.logger(
        "You can specify another port using the --port flag, e.g.:",
        "fail"
      );
  ...
  public async storagePathCheck() {
    if (
      platform() === "darwin" &&
      !(await pathExists(dirname(this.storagePath)))
    ) {
      this.logger(
        `Cannot create Homebridge storage directory, base path does not exist: ${dirname(this.storagePath)}.`,
        "fail"
      );
      process.exit(1);
    }
  ...
  public async configCheck() {
    ...
    if (!(await pathExists(process.env.UIX_CONFIG_PATH))) {
      this.logger(
        `Creating default config.json: ${process.env.UIX_CONFIG_PATH}.`
      );
    ...
    try {
      ...
      let uiConfigBlock = currentConfig.platforms.find(
        (x: any) => x.platform === "config"
      );
      ...
      if (!uiConfigBlock) {
        this.logger(
          `Adding missing UI platform block to ${process.env.UIX_CONFIG_PATH}.`,
          "info"
        );
      ...
      if (this.action !== "install" && typeof uiConfigBlock.port !== "number") {
        ...
        this.logger(
          `Added missing port number to UI config: ${uiConfigBlock.port}.`,
          "info"
        );
        saveRequired = true;
        restartRequired = true;
      ...
      if (this.action === "install") {
        ...
        if (uiConfigBlock.port !== this.uiPort) {
          ...
          this.logger(
            `Homebridge UI port in ${process.env.UIX_CONFIG_PATH} changed to: ${this.uiPort}.`,
            "warn"
          );
        }
      ...
      if (!currentConfig.bridge) {
        ...
        this.logger(
          "Added missing Homebridge bridge section to the config.json.",
          "info"
        );
        saveRequired = true;
      }
      ...
      if (!currentConfig.bridge.port) {
        ...
        this.logger(
          `Added port to the Homebridge bridge section of the config.json: ${currentConfig.bridge.port}.`,
          "info"
        );
        saveRequired = true;
      }
      ...
      if (
        (uiConfigBlock && currentConfig.bridge.port === uiConfigBlock.port) ||
        currentConfig.bridge.port === 8080
      ) {
        currentConfig.bridge.port = await this.generatePort();
        this.logger(
          `Bridge port must not be the same as the UI port. Changing bridge port to: ${currentConfig.bridge.port}.`,
          "info"
        );
        saveRequired = true;
      }
      ...
      if (currentConfig.plugins && Array.isArray(currentConfig.plugins)) {
        if (!currentConfig.plugins.includes("homebridge-config-ui-x")) {
          ...
          this.logger(
            "Added Homebridge UI to the plugins array in the config.json.",
            "info"
          );
          saveRequired = true;
        }
      ...
      if (saveRequired) {
        await writeJson(process.env.UIX_CONFIG_PATH, currentConfig, {
          spaces: 4,
        });
      }
    } catch (e) {
      const backupFile = resolve(
        this.storagePath,
        `config.json.invalid.${new Date().getTime().toString()}`
      );
      this.logger(
        `${process.env.UIX_CONFIG_PATH} does not contain valid JSON.`,
        "warn"
      );
      this.logger(
        `Invalid config.json file has been backed up to ${backupFile}.`,
        "warn"
      );
  ...
  public async createDefaultConfig() {
    await writeJson(
      process.env.UIX_CONFIG_PATH,
      {
        ...
        accessories: [],
        platforms: [await this.createDefaultUiConfig()],
      },
      { spaces: 4 }
    );
  ...
  private async generateBridgeConfig() {
    ...
    const advertiser = (await this.isAvahiDaemonRunning())
      ? "avahi"
      : "bonjour-hap";
  ...
  private async getLastKnownUiPort() {
    ...
    if (await this.isRaspbianImage()) {
      const lastPort = Number.parseInt(
        await readFile("/etc/hb-ui-port", "utf8"),
        10
      );
  ...
  private async generatePort() {
    const randomPort = () =>
      Math.floor(Math.random() * (52000 - 51000 + 1) + 51000);
  ...
  private async isAvahiDaemonRunning(): Promise<boolean> {
    ...
    if (
      !(await pathExists("/etc/avahi/avahi-daemon.conf")) ||
      !(await pathExists("/usr/bin/systemctl"))
    ) {
      return false;
    }
    try {
  ...
  private async checkForStaleHomebridgeProcess() {
    ...
    try {
      ...
      if (
        !(await tcpCheck(
          Number.parseInt(currentConfig.bridge.port.toString(), 10)
        ))
      ) {
        return;
      }
      ...
      this.logger(
        `Found stale Homebridge process running on port: ${currentConfig.bridge.port}, with PID: ${pid}, killing...`
      );
  ...
  private async tailLogs() {
    if (!existsSync(this.logPath)) {
      this.logger(
        `Log file does not exist at expected location: ${this.logPath}.`,
        "fail"
      );
      process.exit(1);
    }
    ...
    const logStats = await stat(this.logPath);
    const logStartPosition =
      logStats.size <= 200000 ? 0 : logStats.size - 200000;
    const logStream = createReadStream(this.logPath, {
      start: logStartPosition,
    });
  ...
  private async loadHomebridgeStartupOptions() {
    try {
      if (await pathExists(this.homebridgeStartupOptionsPath)) {
        const homebridgeStartupOptions = await readJson(
          this.homebridgeStartupOptionsPath
        );

        // Check if debug should be enabled
        if (
          homebridgeStartupOptions.debugMode &&
          !this.homebridgeOpts.includes("-D")
        ) {
        ...
        if (
          homebridgeStartupOptions.keepOrphans &&
          !this.homebridgeOpts.includes("-K")
        ) {
        ...
        if (
          homebridgeStartupOptions.insecureMode === false &&
          this.homebridgeOpts.includes("-I")
        ) {
          this.homebridgeOpts.splice(
            this.homebridgeOpts.findIndex((x) => x === "-I"),
            1
          );
        ...
        if (
          process.env.HOMEBRIDGE_DEBUG === "1" &&
          !this.homebridgeOpts.includes("-D")
        ) {
          this.homebridgeOpts.push("-D");
        }
  ...
  private async checkForNodejsUpdates(requestedVersion: string) {
    const versionList = (await axios.get("https://nodejs.org/dist/index.json"))
      .data;
    ...
    if (requestedVersion) {
      const wantedVersion = versionList.find((x) =>
        x.version.startsWith(`v${requestedVersion}`)
      );
      if (wantedVersion) {
        // Check the requested version is greater than v20.18.0
        if (!gte(wantedVersion.version, "20.18.0")) {
          this.logger(
            "Refusing to install Node.js version lower than v16.18.2.",
            "fail"
          );
          return { update: false };
        }
        this.logger(
          `Installing Node.js ${wantedVersion.version} over ${process.version}...`,
          "info"
        );
        ...
        this.logger(
          `v${requestedVersion} is not a valid Node.js version.`,
          "info"
        );
        return { update: false };
      }
    ...
    if (gt(currentLts.version, process.version)) {
      this.logger(
        `Updating Node.js from ${process.version} to ${currentLts.version}...`,
        "info"
      );
      return this.installer.updateNodejs({
    ...
    const latestVersion = versionList.filter(
      (x) => parse(x.version).major === currentMajor
    )[0];

    if (gt(latestVersion.version, process.version)) {
      this.logger(
        `Updating Node.js from ${process.version} to ${latestVersion.version}...`,
        "info"
      );
      return this.installer.updateNodejs({
   ...
   */
  public async extractNodejs(
    targetVersion: string,
    extractConfig: TarOptionsWithAliases
  ) {
  ...
  private async npmPluginManagement(args: any[]) {
    if (!this.enableHbServicePluginManagement) {
      this.logger(
        "Plugin management is not supported on your platform using hb-service.",
        "fail"
      );
      process.exit(1);
    }
